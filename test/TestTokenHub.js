...
function decodeSyncSynPackage(packageBytes) {
    ...
    const bep20Supply = web3.utils.bytesToHex(syncSynPackage[2]);
    const syncFee = web3.utils.bytesToHex(syncSynPackage[3]);
    const expireTime = web3.utils.bytesToHex(syncSynPackage[4]);
    return {syncSender, bep20Addr, bep20Supply, syncFee, expireTime};
}
...
contract('TokenHub', (accounts) => {
    ...
    it('iterate mirror failures', async () => {
        ...
        try {
            ...
            assert.ok(error.toString().includes("bep20 name length should be in [1,32]"));
        }
        ...
        try {
            ...
            assert.ok(error.toString().includes("bep20 symbol length should be in [3,8]"));
        }
        ...
        try {
            ...
            assert.ok(error.toString().includes("bep20 symbol must not contain non-alphabet"));
        }
